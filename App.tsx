...
const Header = ({ 
  ...
  <div className={`flex items-center justify-between p-4 sticky top-0 z-30 transition-colors ${transparent ? 'bg-transparent' : 'bg-white'} ${className}`}>
...
const BottomNav = ({ activeTab, onTabChange }: { activeTab: TabName, onTabChange: (tab: TabName) => void }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 pb-safe pt-2 px-2 flex justify-between items-end z-50 h-20 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
      {tabs.map((tab) => {
        ...
        if (tab.isSpecial) {
           return (
            <button
              ...
              className="relative -top-5 flex flex-col items-center justify-center px-2 group"
            >
              <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-transform group-active:scale-95 border-4 border-slate-50 ${isActive ? 'bg-red-600 text-white' : 'bg-red-500 text-white'}`}>
                  <tab.Icon className="w-6 h-6 stroke-2" />
              </div>
              <span className={`text-[10px] font-bold mt-1 ${isActive ? 'text-red-600' : 'text-red-500'}`}>{tab.label}</span>
            </button>
           )
        }
        ...
        return (
          <button
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={`flex-1 flex flex-col items-center gap-1 pb-4 transition-all duration-300 active:scale-90 ${isActive ? 'text-brand-600' : 'text-slate-400'}`}
          >
...
const HomeScreen = ({ navigate }: { navigate: (screen: ScreenName, params?: any) => void }) => {
  ...
  return (
    <div className="pb-24 min-h-screen bg-slate-50">
      {/* Header Area */}
      <div className="p-5 flex justify-between items-center bg-white sticky top-0 z-20 shadow-sm">
        <div className="flex items-center gap-3">
        ...
        <div className="bg-white border border-slate-100 shadow-sm rounded-full px-3 py-1.5 flex items-center gap-2">
          <span className="font-bold text-slate-900">1000</span>
          <div className="w-4 h-4 rounded-full bg-gradient-to-tr from-yellow-400 to-orange-500" />
        </div>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto px-5 py-4 no-scrollbar">
        <div 
          ...
          className="min-w-[140px] h-[160px] bg-white rounded-3xl p-4 flex flex-col justify-between shadow-sm border border-slate-100 shrink-0 active:scale-95 transition-transform"
        >
          <h3 className="font-bold text-lg leading-tight">Explore<br/>Doha</h3>
        ...
        <div 
          ...
          className="min-w-[140px] h-[160px] bg-[url('https://images.unsplash.com/photo-1551524559-8af4e6624178?auto=format&fit=crop&q=80&w=300')] bg-cover rounded-3xl relative overflow-hidden shrink-0 active:scale-95 transition-transform"
        >
          <div className="absolute inset-0 bg-black/20" />
          <div className="absolute bottom-3 left-3 text-white font-bold text-sm">Time to ski!</div>
        </div>
         <div 
          onClick={() => navigate('feature_placeholder', { title: 'Rewards', icon: 'Zap' })}
          className="min-w-[140px] h-[160px] bg-gradient-to-br from-purple-400 to-blue-500 rounded-3xl p-4 flex flex-col justify-end text-white shrink-0 active:scale-95 transition-transform"
        >
           <div className="font-bold text-sm leading-tight">Your Coins<br/>expire soon!</div>
        </div>
      </div>
      ...
      <div className="px-5 mt-6">
        <div className="bg-indigo-50 rounded-[32px] p-6 relative overflow-hidden">
          <div className="flex justify-between items-start z-10 relative">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">JETOUR T2</h2>
              <button 
                onClick={() => navigate('insurance_detail')}
                className="mt-6 bg-brand-500 text-white px-6 py-3 rounded-2xl font-semibold shadow-lg shadow-brand-500/30 active:scale-95 transition-transform"
              >
                Insure
              </button>
            </div>
          ...
          <img 
            ...
            alt="Car" 
            className="absolute right-[-40px] top-[20px] w-[220px] object-contain drop-shadow-2xl rotate-1"
          />
          
          <div className="grid grid-cols-2 gap-3 mt-8 z-10 relative">
            <div 
              onClick={() => navigate('city')}
              className="bg-white/60 backdrop-blur-sm p-4 rounded-2xl flex items-center gap-3 active:scale-95 transition-transform cursor-pointer hover:bg-white/80"
            >
              <div className="flex-1">
                <div className="font-bold text-slate-900 leading-tight">Fix. Clean.<br/>Maintain</div>
              </div>
              <Icons.Wrench className="w-6 h-6 text-brand-500" />
            </div>
            <div 
              onClick={() => navigate('market')}
              className="bg-white/60 backdrop-blur-sm p-4 rounded-2xl active:scale-95 transition-transform cursor-pointer hover:bg-white/80"
            >
              <div className="text-xs font-semibold text-slate-500">Buy & sell cars</div>
              <div className="font-bold text-brand-600 text-lg">qic•market</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 mt-8">
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4">
          ...
          >
            ...
            <span className="text-sm opacity-80">Book now -></span>
          </div>
          <div 
          ...
          >
            <div className="font-bold text-lg leading-tight">Contact QIC<br/>help center</div>
             <span className="text-sm opacity-80">Call 24/7 -></span>
          </div>
           <div 
            ...
            <div className="font-bold text-lg leading-tight">Invite<br/>friends</div>
             <span className="text-sm opacity-80">Get coins -></span>
          </div>
        </div>
      </div>
...
const InsuranceScreen = ({ navigate }: { navigate: (screen: ScreenName, params?: any) => void }) => {
  ...
  return (
    <div className="pb-24 min-h-screen bg-slate-50">
      <div className="bg-white p-4 flex justify-between items-end pb-4 border-b border-slate-100 sticky top-0 z-10">
      ...
      <div className="p-4 space-y-3">
        {items.map((item, i) => (
          <div 
            key={i} 
            onClick={() => navigate('insurance_detail')}
            className="bg-white p-4 rounded-2xl flex items-center justify-between shadow-sm active:scale-[0.99] transition-transform"
          >
...
const EmergencyScreen = ({ navigate }: { navigate: (screen: ScreenName, params?: any) => void }) => {
  return (
    <div className="bg-slate-50 min-h-screen pb-24 flex flex-col">
       {/* Red Alert Header */}
       <div className="bg-red-600 px-6 pt-8 pb-12 rounded-b-[40px] shadow-lg relative z-10 text-white">
          ...
          <div className="flex gap-4">
             <button onClick={() => window.location.href = 'tel:999'} className="flex-1 bg-white text-red-600 py-4 rounded-2xl font-bold shadow-md flex items-center justify-center gap-2 active:scale-95 transition-transform">
                <Icons.Phone className="w-5 h-5 fill-red-600" /> Call 999
             </button>
             <button onClick={() => navigate('success_confirmation', { title: 'Accident Reported', message: 'Team dispatched. Stay safe.'})} className="flex-1 bg-red-700 text-white border border-red-500 py-4 rounded-2xl font-bold shadow-md flex items-center justify-center gap-2 active:scale-95 transition-transform">
                <Icons.Alert className="w-5 h-5" /> Report Accident
             </button>
          </div>
       </div>
       ...
       <div className="mx-6 -mt-8 relative z-20 h-[100px] rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
          <MapContainer className="h-full" zoom={1.2}>
             ...
             <div className="absolute inset-0 flex items-center justify-between px-4">
                ...
                <button onClick={() => navigate('success_confirmation', { title: 'Location Shared', message: 'Your coordinates have been sent to emergency contacts.' })} className="p-2 bg-white rounded-full text-slate-400 hover:text-blue-600 shadow-sm active:scale-95 transition-transform">
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="p-6">
        ...
        <div className="bg-brand-50 rounded-3xl p-5 flex gap-4 mb-8">
           <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shrink-0">
             <Icons.Settings className="w-6 h-6 text-brand-600 rotate-45" />
           </div>
           <div>
             <h3 className="font-bold text-slate-900">Pricing Information</h3>
             <p className="text-sm text-slate-600 mt-1 leading-relaxed">These services are <span className="font-bold text-brand-600">Free</span> with your Roadside Plus Add-on. Without the add-on, standard market rates apply.</p>
           </div>
        </div>
          ...
          {[
            ...
            { name: 'Fuel Delivery', sub: 'Emergency fuel top-up', icon: Icons.Fuel },
             ...
             <div 
               ...
               className="bg-white rounded-3xl p-5 flex items-center justify-between shadow-sm active:scale-[0.98] transition-transform cursor-pointer"
             >
               <div>
...
export default function App() {
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen relative shadow-2xl overflow-hidden font-sans text-slate-900 pb-20">
      {renderScreen()}
